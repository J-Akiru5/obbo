...
async function step3_shipments(sb, jb) {
    ...
    if (error) { console.error('❌ Shipments:', error.message); process.exit(1); }
...
async function step4_orders(sb, jb, sbBatch, jbBatch) {
    ...
    for (const client of clients) {
        ...
        const orderDefs = [
            // completed — SB — cash
            {
                status: 'completed', product: sb, batch: sbBatch,
                qty: 100, approved: 100, dispatched: 100,
                payment_method: 'cash', days: 30,
                source: 'port', service_type: 'deliver',
            ...
            // dispatched — JB — check
            {
                status: 'dispatched', product: jb, batch: jbBatch,
                qty: 20, approved: 20, dispatched: 20,
                payment_method: 'check', days: 5,
                source: 'warehouse', service_type: 'pickup',
            ...
            {
                status: 'partially_approved', product: sb, batch: sbBatch,
                qty: 200, approved: 100, dispatched: 0,
                payment_method: 'cash', days: 3,
                source: 'port', service_type: 'deliver',
                tracking_status: 'pending_dispatch',
                notes: 'Split delivery — 100 bags now, 100 bags pending.',
                isSplit: true, deliverNow: 100,
            },
            // pending — SB — cash
            {
                status: 'pending', product: sb, batch: null,
                qty: 50, approved: 0, dispatched: 0,
                payment_method: 'cash', days: 1,
                source: 'port', service_type: 'deliver',
                tracking_status: 'pending_dispatch',
                notes: null, isSplit: false,
            },
            // rejected — JB — cash
            {
                status: 'rejected', product: jb, batch: null,
                qty: 10, approved: 0, dispatched: 0,
                payment_method: 'cash', days: 10,
                source: 'warehouse', service_type: 'pickup',
                tracking_status: 'pending_dispatch',
            ...
            {
                status: 'awaiting_check', product: sb, batch: null,
                qty: 150, approved: 150, dispatched: 0,
                payment_method: 'check', days: 0,
                source: 'port', service_type: 'deliver',
                tracking_status: 'pending_dispatch',
                check_number: `CHK-${Math.floor(Math.random() * 90000) + 10000}`,
                notes: 'Awaiting check clearance.', isSplit: false,
            },
        ];
        ...
        for (const o of orderDefs) {
            ...
            console.log(`    ✅ ${o.status.padEnd(20)} ${o.product.bag_type} × ${o.qty} bags — ₱${totalAmount.toLocaleString()}`);
...
async function step5_ledger(sbBatch, jbBatch, totalDispatchedSB, totalDispatchedJB) {
    ...
    const entries = [
        ...
        {
            shipment_id: sbBatch.id, date: today,
            po_number: 'PO-INIT-SB-001', client_name: 'OBBO Supplier',
            jb: 0, sb: 5000, bags_returned: 0,
            notes: 'Initial SB inventory received.',
        },
        {
            shipment_id: sbBatch.id, date: today,
            dr_number: drNum(), client_name: 'Metro Hardware',
            jb: 0, sb: -Math.min(totalDispatchedSB, 500), bags_returned: 0,
            notes: 'Dispatched to clients.',
        },
        // JB Batch — opening + dispatches
        {
            shipment_id: jbBatch.id, date: today,
            po_number: 'PO-INIT-JB-001', client_name: 'OBBO Supplier',
            jb: 500, sb: 0, bags_returned: 0,
            notes: 'Initial JB inventory received.',
        },
        {
            shipment_id: jbBatch.id, date: today,
            dr_number: drNum(), client_name: 'City Build Corp',
            jb: -Math.min(totalDispatchedJB, 50), sb: 0, bags_returned: 0,
            notes: 'Dispatched to clients.',
        },
    ];
...
async function main() {
    ...
    console.log(`║   SB Batch  :  5,000 bags  (${5000 - totalDispatchedSB} remaining)`.padEnd(49) + '║');
    console.log(`║   JB Batch  :  500 bags    (${500 - totalDispatchedJB} remaining)`.padEnd(49) + '║');
