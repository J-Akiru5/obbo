...
async function seedTransactions() {
  ...
  if (!clients || clients.length === 0) {
    console.error(
      '❌ No clients found to associate with transactions. Please run seed-mock-data.mjs first.',
    );
    return;
  }
  ...
  for (const client of clients) {
    ...
    for (const o of orders) {
      ...
      if (o.status === 'completed') {
        ...
        approved = o.qty;
        dispatched = o.deliver_now_qty;
      } else if (o.status === 'partially_approved') {
        approved = o.deliver_now_qty;
      }
      ...
      if (o.status === 'partially_approved' && o.is_split_delivery) {
        ...
        const remainingQty = (o.qty - o.deliver_now_qty) * equiv;
      ...
      if (o.status === 'completed' || o.status === 'dispatched') {
        await supabaseAdmin.from('delivery_receipts').insert({
          ...
          received_date:
            o.status === 'completed'
              ? new Date(tsNow - (o.days - 2) * 86400000).toISOString()
