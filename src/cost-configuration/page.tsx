...
export default function CostConfigurationPage() {
  ...
  const [localExpenses, setLocalExpenses] = useState<number>(20.00); // Default panel criteria [cite: 33]
  ...
  const sampleSellingPrice = 185.00; // [cite: 35]
  
  // Dynamic formulas base sa inyong document guidelines [cite: 92, 98, 99]
  const totalSales = sampleQty * sampleSellingPrice;
    ...
    <div style={{ padding: "24px", fontFamily: "sans-serif", maxWidth: "900px", margin: "0 auto" }}>
      <div style={{ background: "#fff", border: "1px solid #e2e8f0", borderRadius: "12px", padding: "24px", boxShadow: "0 1px 3px rgba(0,0,0,0.05)" }}>
        ...
        <p style={{ fontSize: "14px", color: "#64748b", margin: "0 0 24px 0" }}>
          These values drive Sales, Gross Profit, and Net Profit across the ledger, dashboard, and reports. 
          Changes only affect new dispatches/orders — past records keep their snapshotched values. [cite: 96, 97]
        </p>

        {message && (
          <div style={{ padding: "12px", borderRadius: "6px", marginBottom: "16px", backgroundColor: "#f0fdf4", color: "#166534", fontSize: "14px", fontWeight: "500" }}>
            {message}
          </div>
        )}
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "20px", marginBottom: "24px" }}>
          <div>
            ...
            <input
              ...
              style={{ width: "100%", padding: "10px", borderRadius: "8px", border: "1px solid #cbd5e1", fontSize: "16px" }}
            />
            <span style={{ fontSize: "12px", color: "#94a3b8", display: "block", marginTop: "4px" }}>
              Base 85.80 + Freight 27.84 + Duties 22.00 + Port Handling 12.00 = 147.64. [cite: 44, 45, 46, 47, 48]
            </span>
          </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "16px", marginBottom: "24px" }}>
          <div style={{ background: "#f8fafc", padding: "16px", borderRadius: "8px", border: "1px solid #e2e8f0" }}>
            <span style={{ fontSize: "12px", color: "#64748b", fontWeight: "500" }}>Gross Profit / bag</span>
            <div style={{ fontSize: "20px", fontWeight: "700", color: "#0f172a", marginTop: "4px" }}>₱{grossProfitPerBag.toFixed(2)}</div>
          </div>
          
          <div style={{ background: "#f8fafc", padding: "16px", borderRadius: "8px", border: "1px solid #e2e8f0" }}>
            <span style={{ fontSize: "12px", color: "#64748b", fontWeight: "500" }}>Net Profit / bag</span>
            <div style={{ fontSize: "20px", fontWeight: "700", color: "#ea580c", marginTop: "4px" }}>₱{netProfitPerBag.toFixed(2)}</div>
          </div>

          <div style={{ background: "#f8fafc", padding: "16px", borderRadius: "8px", border: "1px solid #e2e8f0" }}>
            <span style={{ fontSize: "12px", color: "#64748b", fontWeight: "500" }}>Total Cost / bag</span>
            <div style={{ fontSize: "20px", fontWeight: "700", color: "#0f172a", marginTop: "4px" }}>₱{totalCostPerBag.toFixed(2)}</div>
          </div>
        </div>

        {/* Formula Box */}
        <div style={{ background: "#fff7ed", border: "1px solid #ffedd5", borderRadius: "8px", padding: "16px", marginBottom: "24px", fontSize: "13px", color: "#c2410c" }}>
          <strong style={{ display: "block", marginBottom: "6px" }}>ℹ️ Formulas Applied dynamically:</strong>
          ...
          <div>• Net Profit = Total Sales − (qty × (landed_cost + local_expenses)_at_dispatch) [cite: 99]</div>
        </div>
        ...
        <button
          onClick={handleSave}
          disabled={loading}
          style={{ background: "#f97316", color: "#fff", padding: "12px 24px", border: "none", borderRadius: "8px", fontSize: "15px", fontWeight: "600", cursor: "pointer", display: "flex", alignItems: "center" }}
        >
