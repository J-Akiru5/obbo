...
export async function createRoleNotificationAdmin({
    targetRole,
    title,
    message,
    href = null,
    ...
    if (userError) {
        console.error("Error fetching users for admin notification:", userError);
        return { error: userError.message };
    }
    ...
    // 3. Insert notifications in bulk (bypasses RLS via service-role key)
    const { error: insertError } = await supabase
        .from("notifications")
        .insert(notifications);
