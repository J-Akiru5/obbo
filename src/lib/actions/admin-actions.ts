...
async function createOrderForClientPortal(
  ...
  if (params.jbQty > 0 && jbProduct) {
    orderItems.push({
      ...
      selling_price_per_bag: jbProduct.price_per_bag,
    });
  }
  if (params.sbQty > 0 && sbProduct) {
    orderItems.push({
      ...
      selling_price_per_bag: sbProduct.price_per_bag,
    });
  }
...
export async function generateDailyReportData(date: string) {
  ...
  const dispatches = (orders || []).map((o) => {
    const jb =
      o.items
        ?.filter((i: any) => i.bag_type === 'JB')
        .reduce((s: number, i: any) => s + i.dispatched_qty, 0) || 0;
    const sb =
      o.items
        ?.filter((i: any) => i.bag_type === 'SB')
        .reduce((s: number, i: any) => s + i.dispatched_qty, 0) || 0;
    return {
