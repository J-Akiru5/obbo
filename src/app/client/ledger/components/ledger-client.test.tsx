...
describe('LedgerClient - Balance Ledger Component', () => {
  ...
  it('shows "Completed" badge when remaining balance is 0', () => {
    ...
    // Each balance row renders twice — once in the desktop table, once in
    // the mobile card list (Tailwind's `sm:` classes hide one or the
    // other via CSS, which jsdom doesn't evaluate) — so query for all
    // matches rather than assuming a single element.
  ...
  it('shows request button when balance is greater than 0 and no pending redelivery', () => {
    ...
    // Renders once per layout (desktop table + mobile card) — see note above.
    expect(screen.getAllByText('Request Balance Delivery').length).toBeGreaterThan(0);
  });
  ...
  it('shows disabled pending button when balance has an active redelivery', () => {
    ...
    // Renders once per layout (desktop table + mobile card) — see note above.
    expect(screen.getAllByText('Re-delivery Pending').length).toBeGreaterThan(0);
    expect(screen.queryByText('Request Balance Delivery')).not.toBeInTheDocument();
  });
});
