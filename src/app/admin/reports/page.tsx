...
export default function AdminReportsPage() {
  ...
  }>({ totalSales: 0, totalGrossProfit: 0, totalNetProfit: 0, entries: [] });
  ...
  const _clientObligations = useMemo(() => {
    const obligations: Record<string, { clientName: string; jb: number; sb: number }> = {};
    for (const balance of balances) {
      const clientName =
        balance.client?.company_name || balance.client?.full_name || 'Unknown Client';
    ...
    <div className="mx-auto flex w-full max-w-7xl flex-col gap-6">
        ...
        <div className="space-y-2">
          ...
          <div>
            <h2 className="text-foreground text-2xl font-bold tracking-tight">
              Warehouse report review
            </h2>
            <p className="text-muted-foreground mt-2 max-w-3xl text-sm leading-6">
              View-only reporting for daily inventory reconciliation, movement tracking, and client
          ...
          <section className="grid gap-6 lg:grid-cols-2">
            <Card className="border-border shadow-sm">
              <CardHeader className="border-border/60 border-b pb-4">
                ...
                <CardDescription>
                  Auto-generated from dispatched and completed orders for the selected date.
                </CardDescription>
              </CardHeader>
              <CardContent className="p-0">
                <Table>
            ...
            <Card className="border-border shadow-sm">
              <CardHeader className="border-border/60 border-b pb-4">
                <CardTitle className="text-base font-semibold">
                  Customer obligation report
                </CardTitle>
                <CardDescription>
                  Pending balances owed by clients, grouped for oversight.
                </CardDescription>
              </CardHeader>
              <CardContent className="p-0">
                <Table>
      ...
      <div className="border-border space-y-6 border-t pt-6">
        <div className="flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center">
          <div>
            <h3 className="text-foreground text-lg font-bold">Sales & Profit Report</h3>
          ...
          <Card className="border-border shadow-sm">
            <CardContent className="p-5 text-center">
              ...
              <p className="text-foreground text-3xl font-bold">
                ₱{profitReport.totalSales.toLocaleString()}
              </p>
            </CardContent>
          </Card>
          <Card className="border-border shadow-sm">
            <CardContent className="p-5 text-center">
              ...
              <p className="text-3xl font-bold text-emerald-600">
                ₱{profitReport.totalGrossProfit.toLocaleString()}
              </p>
            </CardContent>
          </Card>
          <Card className="border-border shadow-sm">
            <CardContent className="p-5 text-center">
              ...
              <p
                className={`text-3xl font-bold ${profitReport.totalNetProfit >= 0 ? 'text-blue-600' : 'text-red-500'}`}
              >
                  ...
                  {profitReport.entries.map((entry, idx) => (
                    <TableRow key={idx}>
                      ...
                      <TableCell className="text-right text-sm">
                        {(
                          (Number(entry.jb) || 0) * 25 +
                          (Number(entry.sb) || 0) * 50
                        ).toLocaleString()}
                      </TableCell>
                      <TableCell className="text-right text-sm font-medium">
                        ₱{(Number(entry.total_sales) || 0).toLocaleString()}
                      </TableCell>
                      <TableCell className="text-right text-sm text-emerald-600">
                        ₱{(Number(entry.gross_profit) || 0).toLocaleString()}
                      </TableCell>
                      <TableCell
                        className={`text-right text-sm font-bold ${(Number(entry.net_profit) || 0) >= 0 ? 'text-blue-600' : 'text-red-500'}`}
                      >
